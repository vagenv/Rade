...
void ARadePlayer::BeginPlay()
{
	...
	if (  ThePC
		&& ThePC->GetHUD()
		&& Cast<ABaseHUD>(ThePC->GetHUD()))
		TheHUD = Cast<ABaseHUD>(ThePC->GetHUD());

	// Manage Inventory
	if (TheInventory) {
		// Load Inventory
		if (bSaveInventory && GetLocalRole() >= ROLE_Authority)
		{
	...
	// Enable and start jet pack FillUp
...
void ARadePlayer::Action_Implementation()
{
	...
	{
		if (TheInventory) TheInventory->ActionIndex(CurrentItemSelectIndex);
...
void ARadePlayer::FAction_Implementation()
{
	...
	// Throw out Current item selected
	if (  TheInventory
		&& bInventoryOpen
		&& IsAnimState(EAnimState::Idle_Run))
	{
...
void ARadePlayer::Jump()
{
	...
	} else if (  bJetPackEnabled
				 && bCanFillJetPack
				 && JumpJetPack.CurrentChargePercent > JumpJetPack.MinUseablePercent)
	{
		DoubleJump();
...
void ARadePlayer::MouseScroll(float Value)
{
	// Call Scroll on HUD
	if (TheHUD) {
		if      (Value > 0) TheHUD->BP_InventoryScrollUp();
		else if (Value < 0) TheHUD->BP_InventoryScrollDown();
...
void ARadePlayer::Landed(const FHitResult& Hit)
{
	...
	if (!bCanFillJetPack) bCanFillJetPack = true;
...
void ARadePlayer::SetCharacterStats_Implementation(const FString & newName, FLinearColor newColor)
{
	...
	if (GetPlayerState()) {
		...
		if (Cast<ARadePlayerState>(GetPlayerState()))Cast<ARadePlayerState>(GetPlayerState())->PlayerColor = newColor;
	}
}
